...
However an arbitrary incidence geometry has no notion of ``order'' for the points on a given line, as we intuitively expect.
...
\begin{prop}
Suppose \(\mathcal{P}\) is an incidence geometry and \(\BETWEEN{\cdot}{\cdot}{\cdot}\) a betweenness relation with the Trichotomy Property.
...
A non empty set \(S\) of points in \(\mathcal{P}\) is called \emph{convex}\index{convex} if it is closed under betweenness in the following sense: if \(x,y \in S\) and \(\BETWEEN{x}{z}{y}\), then \(z \in S\).
\end{dfn}
...
\begin{exercise}
Let \(P\) be an incidence geometry and \(x,y \in P\) distinct points.
