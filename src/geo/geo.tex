\documentclass{memoir}

\input{src/packages.tex}
\input{src/style.tex}
\input{src/macros.tex}

\makeindex

\begin{document}

\title{Elementary Geometrese}
\author{nathan bloomfield}


\frontmatter

\thispagestyle{empty}

\begin{center}
{\Large Elementary Geometrese}

\vspace{1cm}

Nathan Bloomfield

\vspace{3cm}

\includegraphics[scale=1.8,trim=4 4 4 4,clip]{src/geo/gfx/cover/cover.eps}
\end{center}

\newpage
\tableofcontents

\setcounter{page}{0}

\begin{center}
\begin{minipage}{0.8\textwidth}
\begin{center}
\vspace{40pt}

This work is licensed under a Creative Commons Attribution NonCommercial ShareAlike 4.0 International License.

\vspace{12pt}

\includegraphics[scale=0.8]{src/geo/gfx/by-nc-sa.eps}
\end{center}
\end{minipage}
\end{center}
\newpage


\mainmatter

\chapter{Incidence}
\newpage

  \section{Models and Theories}
    \input{src/geo/1.01-models-and-theories.tex}
    \newpage

  \section{Incidence Geometry}
    \input{src/geo/1.02-incidence-geometry.tex}
    \newpage

  \section{Parallel Lines}
    \input{src/geo/1.03-parallel-lines.tex}
    \newpage

  \section{Collineations}
    \input{src/geo/1.04-collineations.tex}


\chapter{Order}
\newpage

  \section{Betweenness}
    \input{src/geo/2.01-betweenness.tex}
    \newpage

  \section{Ordered Geometry}
    \input{src/geo/2.02-ordered-geometry.tex}
    \newpage

  \section{Angles}
    \input{src/geo/2.03-angles.tex}


\chapter{Neutral Plane Geometry}
\newpage

  \section{Congruence}
    \input{src/geo/3.01-congruence.tex}
    \newpage

  \section{Plane Geometry}
    \input{src/geo/3.02-plane-geometry.tex}
    \newpage

  \section{Transversals}
    \input{src/geo/3.03-transversals.tex}
    \newpage

  \section{Perpendiculars and Tangents}
    \input{src/geo/3.04-perpendiculars-and-tangents.tex}
    \newpage

  \section{Incircles and Excircles}
    \input{src/geo/3.05-incircles-and-excircles.tex}


\chapter{Euclidean Planes}
\newpage

  \section{Euclidean Planes}
    \input{src/geo/4.01-euclidean-planes.tex}


\chapter{Models}
\newpage

  \section{\(\RR^2\) -- The Cartesian Plane}
    \input{src/geo/A.01-cartesian-plane-model.tex}
    \newpage

  \section{\(\HYPHP\) -- The Hyperbolic Half Plane}
    \input{src/geo/A.02-hyperbolic-half-plane-model.tex}
    \newpage

  \section{\(\POINCAREDISC\) -- The Poincare Disc}
    \input{src/geo/A.03-poincare-disc-model.tex}
    \newpage

  \section{\(\KLEINDISC\) -- The Klein Disc}
    \input{src/geo/A.04-klein-disc-model.tex}
    \newpage

  \section{\(\GANSDISC\) -- The Gans Disc}
    \input{src/geo/A.05-gans-disc-model.tex}


\backmatter
  \bibliographystyle{plain}
  \bibliography{src/bibliography}

  \printindex

\end{document}
