...
For example, we can use it to establish the following ``obvious'' result.
...
It is not too hard to show that every integer \(a\) satisfies exactly one of \(a > 0\), \(a = 0\), and \(a < 0\), a result we call the Trichotomy property (and prove in \eref{exerc:trichotomy}).
...
\begin{exercise}[Trichotomy Property of \(\ZZ\).]\label{exerc:trichotomy}
Show that if \(a \in \ZZ\), then one and only one of \(a < 0\), \(a = 0\), or \(a > 0\) is true.
\end{exercise}
